...
CATEGORIES = {
    ...
    "childrens_early": {
        ...
        "topics": ["talking puppy", "treehouse club", "midnight bakery mice"],
    },
    "childrens_chapter": {
...
def true_crime_system_prompt(agent_name):
    return "You are %s, a factual true-crime narrator. Write ONE public case only. No invented dialogue or motive." % agent_name

def normal_system_prompt(agent_name, category_key="romance"):
    if is_childrens(category_key):
        age = CATEGORIES[category_key]["age"]
        return "You are %s. Write an original children's book for %s. Match vocabulary to that age. No adult content." % (agent_name, age)
...
def story_parts(text):
    ...
    start = ("chapter 1" in low) or any(x in low[:2500] for x in [
        "once upon", "it started", "one night", "one day", "prologue", "first", "velvet lantern", "pip the mouse"
    ])
    middle = word_count(text) >= 120 and any(x in low for x in [
        "but", "then", "suddenly", "problem", "until", "secret",
        "realized", "almost", "climax", "chase", "afraid", "lost", "ghost", "gino", "inspector"
    ])
    ending = any(x in low[-3500:] for x in [
        ...
        "fin.", "at last", "safe again", "crimson vow", "open for keeps"
    ])
...
def build_outline_prompt(agent_name, category_key, topic, cat):
    ...
    if is_childrens(category_key):
        return [
            ...
            {"role": "user", "content": "Plan a children's book for %s about %s. Include title, beginning, middle problem/climax, and ending. Style: %s." % (cat["age"], topic, cat["style"])}
        ]
    return [
        {"role": "system", "content": normal_system_prompt(agent_name, category_key)},
        {"role": "user", "content": "Create title, blurb, characters, and a chapter outline for an original %s book about %s. The outline must have beginning, middle climax, and ending." % (category_key, topic)}
    ]
...
def build_chapter_prompt(agent_name, category_key, topic, cat, chapter, previous):
    ...
    if category_key == "true_crime":
        return [
            {"role": "system", "content": true_crime_system_prompt(agent_name)},
            {"role": "user", "content": "Continue the one chosen case. Chapter %s. Assignment: %s. Public facts only.\nContinue from:\n%s" % (chapter, topic, previous)}
        ]
    if is_childrens(category_key):
        return [
            {"role": "system", "content": normal_system_prompt(agent_name, category_key)},
            {"role": "user", "content": "Write Chapter %s of a %s book about %s.\nStyle: %s\nKeep writing the story.\nContinue from:\n%s" % (chapter, cat["age"], topic, cat["style"], previous)}
        ]
    return [
        {"role": "system", "content": normal_system_prompt(agent_name, category_key)},
        {"role": "user", "content": "Write Chapter %s of this %s book about %s. Style: %s.%s Use dialogue and description. Keep Luca Dante and Evie consistent. No thinking notes.\nContinue from:\n%s" % (chapter, category_key, topic, cat["style"], extra, previous)}
    ]

def build_ending_prompt(agent_name, category_key, topic, previous):
    cat = CATEGORIES.get(category_key, {})
    if is_childrens(category_key):
        ask = "Write the FINAL chapter of this %s children's book about %s. Resolve the problem. Give a clear ending. End with The End.\nContinue from:\n%s" % (cat.get("age", "children"), topic, previous)
    elif category_key == "true_crime":
        ask = "Write the FINAL chapter using only public facts. Cover the latest public outcome.\nContinue from:\n%s" % previous
    else:
        ask = "Write the FINAL chapter of this %s book about %s. Hit the climax if needed, then end the story. Keep Luca Dante and Evie if this is spicy romance.\nContinue from:\n%s" % (category_key, topic, previous)
    return [
        {"role": "system", "content": normal_system_prompt(agent_name, category_key)},
...
def write_full_novel(agent_name, category_key, topic, max_chapters=4):
    ...
    try:
        outline = generate_with_fallback(build_outline_prompt(agent_name, category_key, topic, cat), temperature=0.2 if category_key == "true_crime" else 0.9)
    except Exception as e:
    ...
    for chapter in range(1, chapters + 1):
        ...
        while word_count(chapter_text) < need and tries < 6:
            tries += 1
            try:
                chunk = generate_with_fallback(build_chapter_prompt(agent_name, category_key, topic, cat, chapter, previous), temperature=0.2 if category_key == "true_crime" else 0.9)
            except Exception as e:
    ...
    info = {"agent": agent_name, "category": category_key, "topic": topic, "file": filename, "words": final_words, "chapters": last_chapter, "created_at": timestamp}
...
def latest_book_to_continue():
    ...
    for path in root.rglob("*.txt"):
        ...
        if "refined" in name or name in ("category.json",):
            continue
        if "midnight_bakery_mice" in name:
            continue
...
def continue_book(path, extra_chapters=6):
    ...
    for chapter in range(start_chapter, start_chapter + extra_chapters):
        ...
        while word_count(chapter_text) < need and tries < 6:
            tries += 1
            try:
                chunk = generate_with_fallback(build_chapter_prompt(agent_name, category_key, topic, cat, chapter, previous), temperature=0.2 if category_key == "true_crime" else 0.85)
            except Exception as e:
    ...
    if word_count(text_now) >= need_words and not parts["ending"]:
        try:
            ending = generate_with_fallback(build_ending_prompt(agent_name, category_key, topic, previous), temperature=0.7)
...
def write_book_status():
    ...
    if not rows:
        ...
        for row in rows:
            lines.append("- %s | %s | %s/%s words | ch %s | begin=%s mid=%s end=%s | %s" % (
...
def load_last_toku_run():
    p = Path("toku/last_run.json")
    if not p.exists():
        return {"new_bids": 0, "results": []}
    try:
        return json.loads(p.read_text(encoding="utf-8"))
    except Exception:
        return {"new_bids": 0, "results": []}

def new_applied_lines():
    data = load_last_toku_run()
    lines = []
    for row in data.get("results") or []:
        ...
        title = job.get("title") or row.get("title") or "untitled"
        team = row.get("team") or "unknown"
        code = row.get("response_code") or ""
        cents = row.get("priceCents") or 0
        lines.append("%s | code=%s | $%.2f | %s" % (team, code, int(cents) / 100.0, title))
    return lines, int(data.get("new_bids") or 0)
...
def write_fleet_report():
    ...
    if Path("books").exists():
        for path in Path("books").rglob("*.txt"):
            if "refined" in path.name.lower():
                continue
            ...
            books_touched.append("- %s (%s words)" % (path, word_count(path.read_text(encoding="utf-8", errors="ignore"))))
    applied, new_bids = new_applied_lines()
    lines = [
        "# Fleet Report",
        ...
        "Books touched: %s" % len(books_touched),
        "New Toku bids this run: %s" % new_bids,
        "",
        "## Books",
    ]
    lines.extend(books_touched or ["None"])
    lines += ["", "## New Toku bids this run"]
    lines.extend(["- " + x for x in applied] or ["- None"])
...
def write_all_bids():
    ...
    Path("toku-all-bids.md").write_text(
        "# All logged applied bids\n\n" + "\n".join("- " + x for x in applied or ["None"]) + "\n",
        encoding="utf-8",
    )
    print("Wrote toku-all-bids.md")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--all-bids", action="store_true")
