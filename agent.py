...
# Initialize Coinbase CDP Wallet Context via uploaded JSON file
cdp_context = "CDP SDK not active (cdp_api_key.json missing)."
json_key_path = "cdp_api_key.json"

if CDP_AVAILABLE and os.path.exists(json_key_path):
    try:
        Cdp.configure_from_json(json_key_path)
        wallet = Wallet.create()
        cdp_context = f"Active CDP Wallet Address: {wallet.get_address().getId()} | Network: Base-Sepolia | Configured via JSON"
    except Exception as e:
        cdp_context = f"CDP JSON Configuration Error: {e}"
...
for agent in agents:
    ...
    Niche: {agent['niche']}.
    Coinbase CDP On-Chain Context: {cdp_context}
    Recent Network Context: {recent_history}
    
    Tasks:
    1. Draft 1 high-converting video title & caption for social media integrating on-chain awareness.
...
full_report = f"# 🚀 9-Agent Daily Post & Memory Sync (CDP Upgraded)\n\n**Coinbase CDP On-Chain Status:** {cdp_context}\n\n" + "\n\n".join(network_reports)

if github_token and repo:
    issue_url = f"https://api.github.com/repos/{repo}/issues"
    issue_headers = {"Authorization": f"token {github_token}", "Accept": "application/vnd.github.v3+json"}
    requests.post(issue_url, headers=issue_headers, json={"title": "9-Agent Daily Broadcast Report - CDP Upgraded", "body": full_report})
