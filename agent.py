...
for agent in agents:
    ...
    try:
        ...
        # BROADCAST VIDEO POST VIA DIRECT UPLOAD-POST API REQUEST
        broadcast_status = "Skipped (No Social API Key)"
        if social_api_key:
            try:
                upload_headers = {
                    "Authorization": f"Apikey {social_api_key}"
                }
                form_data = {
                    "user": "AgentNetwork1",
                ...
                # Optional: if API accepts remote video URL or file
                # We send standard multipart post
                response = requests.post(
                    "https://api.upload-post.com/api/v1/upload", # Adjust endpoint if needed
                    headers=upload_headers,
                    data=form_data
                )
                if response.status_code in [200, 201]:
                    broadcast_status = "Successfully Broadcasted Video via Upload-Post API"
                else:
                    broadcast_status = f"API error ({response.status_code}): {response.text}"
